 ...
 * @since 2018.10.06
 */
package org.rmj.engr.inventory.base;
...
import org.rmj.engr.inventory.base.views.SubUnitController;
import org.rmj.engr.inventory.others.pojo.UnitInvTransferDetailOthers;
...
public class InvTransfer{
    ...
    public boolean BrowseRecord(String fsValue, boolean fbByCode){
        ...
        String lsColName = "sTransNox»xDestinat»dTransact";
        String lsColCrit = "a.sTransNox»b.sProjDesc»a.dTransact";
        String lsSQL = MiscUtil.addCondition(getSQ_InvTransfer(), "a.sTransNox LIKE " + SQLUtil.toSQL(psBranchCd + "%"));
    ...
    public boolean BrowseAcceptance(String fsValue, boolean fbByCode){
        String lsHeader = "Transfer No»Destination»Date";
        String lsColName = "sTransNox»xSourcexx»dTransact";
        String lsColCrit = "a.sTransNox»c.sProjDesc»a.dTransact";
        String lsSQL = MiscUtil.addCondition(getSQ_InvTransfer(), 
                                                "a.sDestinat = " + SQLUtil.toSQL(psBranchCd));
    ...
    private boolean unpostInvTrans(){
        InventoryTrans loInvTrans = new InventoryTrans(poGRider, poData.getDestinat());
        loInvTrans.InitTransaction();
    ...
    public boolean closeTransaction(String string) {
        ...
        if (!loObject.getTranStat().equalsIgnoreCase(TransactionStatus.STATE_OPEN)){
            setMessage("Unable to CLOSE already processed transaction.");
            return lbResult;
        }
    ...
    public boolean postTransaction(String string) {
        ...
        if (loObject.getTranStat().equalsIgnoreCase(TransactionStatus.STATE_OPEN) ||
            ...
            setMessage("Unable to POST not yet printed transaction.");
            return lbResult;
        }
    ...
    public boolean cancelTransaction(String string) {
        ...
        if (loObject.getTranStat().equalsIgnoreCase(TransactionStatus.STATE_POSTED) ||
                loObject.getTranStat().equalsIgnoreCase(TransactionStatus.STATE_CANCELLED) ||
                loObject.getTranStat().equalsIgnoreCase(TransactionStatus.STATE_VOID)){
            setMessage("Unable to cancel processed transaction.");
            return lbResult;
        }
    ...
    public boolean SearchMaster(int fnCol, String fsValue, boolean fbByCode){       
        switch(fnCol){
            case 2: //sBranchCd --> sProjctNm
                ...
                break;
            case 4: //sDestinat
                XMProject loBranch = new XMProject(poGRider, psBranchCd, true);
                if (loBranch.browseRecord(fsValue, fbByCode)){
                    setMaster(fnCol, (String) loBranch.getMaster("sProjCode"));
                    MasterRetreived(fnCol);
                    return true;
                }
                break;
    ...
    private String getSQ_InvTransfer(){        
        ...
        String lsSQL = "SELECT " +
                            "  a.sTransNox" +
                            ", b.sProjDesc xDestinat" +
                            ", a.dTransact" +
                            ", c.sProjDesc xSourcexx" + 
                        " FROM Inv_Transfer_Master a" +
                            " LEFT JOIN Project b" +
                                " ON a.sDestinat = b.sProjCode" +
                            " LEFT JOIN Project c" +
                                " ON a.sBranchCd = c.sProjCode";
